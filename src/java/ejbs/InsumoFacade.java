...
public class InsumoFacade extends AbstractFacade<GeInsumo> {
    ...
    public List<GeInsumo> listaInsumoSearchLista(String txt) {
    ...
    public List<GeInsumo> listaInsumoSearch(String txt) {
        //System.out.println("listaInsumoSearch "+ txt);
        String txtUpper = txt.toUpperCase();
        Query q = em.createQuery("SELECT d FROM GeInsumo d WHERE d.estadoregistro = 1 and (UPPER(d.nombre) like :txt OR upper(d.descripcion) like :txt OR upper(d.codigo) like :txt) AND d.etapa.id = 2 ORDER BY d.nombre");
        q.setParameter("txt", "%"+txtUpper+"%");
        return q.getResultList();
    }
    
    public boolean actualizarInsumo(GeInsumo GeInsumo) {
        em.merge(GeInsumo);
        em.flush();
        return true;
    }
